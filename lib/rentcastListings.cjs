...
function getTargetCity(value = "") {
  ...
  const matchedCity = Object.keys(CITY_DESTINATIONS).find((city) =>
  ...
  if (normalizedValue.includes("downtown") || normalizedValue.includes("slu")) {
    return "Seattle";
  }
...
async function enrichListings(listings, apiKey, maxListings) {
  ...
  const requestedDiscoveryLimit = Number(
    process.env.LISTING_LINK_DISCOVERY_LIMIT || Math.min(maxListings || 2, 2)
  );
...
async function getHousingListings(options = {}) {
  ...
  const destination = getSearchTarget(options);
  const hasRentCastKey = Boolean(getRentCastKey());

  if (hasLocationSearch && hasRentCastKey) {
    try {
  ...
  if (hasLocationSearch && destination.city !== "Seattle" && !hasRentCastKey) {
    return [];
  }
